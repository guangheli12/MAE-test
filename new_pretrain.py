...
from engine_pretrain_bootstrap import train_one_epoch_new 
...
def main(args):
    ...
    for epoch in tqdm(range(args.start_epoch, args.epochs)):
        ...
        # 定 num_k == warmup_target_epochs 
        if epoch == args.num_k: 
            slow_target = models_mae.__dict__[args.target_model](norm_pix_loss=args.norm_pix_loss)  
            slow_target.to(device) 
            ...
            slow_target.decoder_pred = torch.nn.Linear(model.decoder_embed_dim, model.embed_dim, bias = True).to(device)     
            slow_target.load_state_dict(model.state_dict())
        ...
        train_stats = train_one_epoch_new(
        ...
        if slow_target is not None and epoch % args.num_k == 0: 
            slow_target.load_state_dict(model.state_dict())
